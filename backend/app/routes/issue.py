...
async def issue_lifetime(request: dict, conn=Depends(get_connection)):
    try:
        ...
        if not all_milestones:
            ...
            # get all the active milestones
            print("list of collection", conn.list_collection_names())
            aggregate = [
                {"$match": {"state": "active"}},
                {"$project": {"_id": 0, "id": 1}},
            ]
            result = milestones.aggregate(aggregate)
            print(list(result))
            all_milestones = [milestone["id"] for milestone in list(result)]
