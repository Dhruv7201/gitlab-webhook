    ...
    while True:
        ...
        if response.status_code != 200:
            raise HTTPException(
                ...
                detail="Failed to fetch labels from GitLab API",
            )
        ...
        for label in data:
            ...
            user_label = labels_collection.find_one(
                {"name": label["name"], "type": "user", "enabled": True}
            )
            issue_label = labels_collection.find_one(
                {"name": label["name"], "type": "issue", "enabled": True}
            )
...
async def add_edit_user(user: UserSchema, db=Depends(get_connection)):
    ...
    if existing_user:
        ...
        if not user.email:
            ...
            secret_key = os.getenv("GITLAB_KEY")
            url = (
                f"https://code.ethicsinfotech.in/api/v4/users?username={user.username}"
            )
            headers = {"Private-Token": secret_key}
            ...
            if response.status_code != 200:
                raise HTTPException(
                    status_code=response.status_code,
                    detail="Failed to fetch user details from GitLab API",
                )
            data = response.json()
