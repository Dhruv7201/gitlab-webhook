...
async def release_plan(
    ...
    query = {
        "work.label": "ReleasePlan",
        "work.start_time": {"$lt": selected_date}
    }
    all_issues = list(issues_collection.find(query))

    projects_collection = conn["projects"]
    all_projects = {project['id']: project['name'] for project in projects_collection.find()}
    ...
    for issue in all_issues:
        ...
        project_list = ['id', 'iid', 'title', 'due_date', 'url', 'created_at', 'project_name', 'status']
