...
def get_gitlab_subgroup(project_id: int) -> str:
    try:
        ...
        response = requests.get(
            f"{gitlab_url}/projects/{project_id}", headers={"PRIVATE-TOKEN": api_token}
        )
        ...
        for project in projects:
            if "subgroup_name" not in project:
                ...
                projects_collection.update_one(
                    {"_id": project["_id"]}, {"$set": {"subgroup_name": subgroup_name}}
                )
                full_project_name = f"{subgroup_name}/{project['name']}"
                user_response.append(
                    {"id": str(project["id"]), "name": full_project_name}
                )
            else:
            ...
            "data": [
                {
                    ...
                    "time_spend_on_project": format_duration(
...
async def get_donut_labels(request: dict, conn=Depends(get_connection)):
    try:
        ...
        labels = []
        for res in result:
            labels.append({"label": res["_id"], "count": res["count"]})
    ...
    try:
        ...
        pipeline = [
            ...
            {"$unwind": "$work"},
            {
                "$addFields": {
                    "areFieldsEqual": {
                        "$eq": ["$assign_issues.issue_id", "$work.issue_id"]
                    }
                }
            },
            {"$match": {"areFieldsEqual": True}},
            {
                "$match": {
                    "$expr": {
                        "$and": [
            ...
            {"$sort": {"total_time_waste": 1}},
            {
                "$project": {
                    "username": "$_id",
                    "user_id": "$id",
                    "total_time_waste": {"$divide": ["$total_time_waste", 1000]},
                }
            },
        ]
        ...
        response = [
            {
                "name": res["username"],
                "user_id": res["user_id"],
                "time_waste": res["total_time_waste"],
            }
            for res in results
        ]
        return {
        ...
        data = [
            {
                ...
                "percentage": (res["duration"] * 100) / total_time
                if total_time > 0
                else 0,
            }
            for res in result
...
def get_work_duration_time(request: Dict[str, Any], conn=Depends(get_connection)):
    try:
        ...
        total_time = sum(
            res["duration"] for res in result if res["duration"] is not None
        )
        ...
        return {
            "status": True,
            "data": data,
            "message": "Work and duration for user retrieved successfully",
        }
        ...
        return {
            "status": False,
            "data": [],
            "message": f"Error in getting work and duration for user: {str(e)}",
        }
...
def get_user_all_info(request: dict, conn=Depends(get_connection)):
    try:
        ...
        response = requests.get(
            f'https://code.ethicsinfotech.in/api/v4/users?username={result["username"]}',
            headers=headers,
        )
        ...
        if response.status_code != 200:
            return {
                "status": False,
                "data": [],
                "message": f"Error fetching GitLab user info: {response.status_code}",
            }
        ...
        try:
            ...
            if not response_data:
                return {
                    "status": False,
                    "data": [],
                    "message": "No data returned from GitLab API",
                }
            ...
            return {
                "status": False,
                "data": [],
                "message": "Invalid JSON response from GitLab API",
            }

        return {
            "status": True,
            "data": result,
            "message": "Work and duration for user got Successfully",
        }
    except Exception as e:
        return {"status": False, "data": [], "message": str(e)}
