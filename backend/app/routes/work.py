...
async def get_work(request: dict, conn=Depends(get_connection)):
    try:
        ...
        aggregate = [{"$unwind": "$work"}]

        match_conditions = {
            "work.start_time": {"$gte": start_date, "$lte": end_date},
            "work.end_time": {"$gte": start_date, "$lte": end_date, "$ne": None},
        }
...
async def get_work_duration(request: dict, conn=Depends(get_connection)):
    try:
        ...
        aggregate = [
            {"$unwind": "$work"},
            {"$match": {"work.end_time": {"$ne": None}, "work.project_id": project_id}},
            {"$group": {"_id": "$name", "total_duration": {"$sum": "$work.duration"}}},
        ]
        result = user_collection.aggregate(aggregate)
        return {
            "status": True,
...
async def get_user_work_done_list(request: dict, conn=Depends(get_connection)):
    try:
        ...
        aggregate += [
            ...
            {
                "$group": {
                    "_id": {"username": "$name", "issue_id": "$work.issue_id"},
                    "total_duration": {"$sum": "$work.duration"},
            ...
            {"$project": {"_id": 0, "username": "$_id", "issues": 1}},
        ]
        result = user_collection.aggregate(aggregate)
        return {
            "status": True,
...
async def get_assignee_task_list(request: dict, conn=Depends(get_connection)):
    try:
        ...
        if project_id != 0:
            aggregate.append({"$match": {"project_id": project_id}})

        aggregate += [
            {"$unwind": "$assign"},
            {"$match": {"assign.start_time": {"$gte": start_date, "$lte": end_date}}},
            {
                "$lookup": {
                    "from": "users",
        ...
        # copy all data from milestones to milestone
        milestones_collection = conn["milestones"]
        milestone_collection = conn["milestone"]

        milestones = list(milestones_collection.find())
        milestone_collection.drop()
        milestone_collection.insert_many(milestones, ordered=False)
...
def get_user_by_work(request: dict, conn=Depends(get_connection)):
    ...
    for individual_data in result:
        individual_data["label_info"]["percentage"] = (
            individual_data["label_info"]["duration"] * 100
        ) / total_time
    data = []
    for res in result:
        data.append(res)
    return {
        "status": True,
        "data": data,
        "message": "Successfully return user with total_duration and time",
    }
...
async def get_daily_work_report(conn=Depends(get_connection)):
    ...
    aggregate = [
        ...
        {"$unwind": {"path": "$assign_info", "preserveNullAndEmptyArrays": True}},
        {
            "$group": {
                "_id": {"user_name": "$name", "issue_id": "$work.issue_id"},
    ...
    for item in data:
        ...
        item["assign_start_time"] = (
            item["assign_start_time"].strftime("%Y-%m-%d %H:%M:%S")
            if item["assign_start_time"]
            else None
        )
        item["assign_end_time"] = (
            item["assign_end_time"].strftime("%Y-%m-%d %H:%M:%S")
            if item["assign_end_time"]
            else None
        )
        item["work_duration"] = (
            item["work_duration"] / 3600 if item["work_duration"] else None
        )
        item["assign_duration"] = (
            item["assign_duration"] / 3600 if item["assign_duration"] else None
        )
        item["work_duration"] = (
            round(item["work_duration"], 2) if item["work_duration"] else None
        )
        item["assign_duration"] = (
            round(item["assign_duration"], 2) if item["assign_duration"] else None
        )
