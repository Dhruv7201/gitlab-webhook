...
def insert_project(project: project_model, db):
    ...
    if project_collection.find_one({"id": project["id"]}):
        if (
            project_collection.find_one({"id": project["id"]})["description"]
            != project["description"]
        ):
            filter = {"id": project["id"]}
            newvalues = {
                "$set": {"description": project["description"], "updated_at": curr_time}
            }
    ...
    for curr_work in curr_work_arr:
        if curr_work["start_time"]:
            ...
            for i in range(len(work_arr) - 1, -1, -1):
                work = work_arr[i]
                if (
                    work["issue_id"] == curr_work["issue_id"]
                    and work["label"] == curr_work["label"]
                    and work["start_time"]
                    and not work["end_time"]
                ):
                    work_arr[i]["end_time"] = curr_work["end_time"]
                    work_arr[i]["duration"] = (
                        curr_work["end_time"] - work["start_time"]
                    ).total_seconds()
                    break
...
def push_milestone(issue_id, curr_milestone_id, prev_milestone_id, updated_at, db):
    ...
    curr_issue = issue_collection.find_one(filter)
    new_milestone = {
        "milestone_id": curr_milestone_id,
    ...
    if "milestone" in curr_issue and prev_milestone_id != None:
        ...
        for index in range(len(curr_milestone_info) - 1, -1, -1):
            if (
                curr_milestone_info[index]["milestone_id"] == prev_milestone_id
                and "end_time" not in curr_milestone_info[index]
            ):
                curr_milestone_info[index]["end_time"] = datetime.now()
                curr_milestone_info[index]["duration"] = (
                    datetime.now() - curr_milestone_info[index]["start_time"]
                ).total_seconds()
                break
...
def push_assign(id, previous_assign, current_assign, project_id, db):
    ...
    if "assign" not in issue or previous_assign == []:
        ...
        if not isinstance(assign_arr[-1]["start_time"], datetime):
            assign_arr[-1]["start_time"] = datetime.strptime(
                start_time, "%Y-%m-%dT%H:%M:%S.%f"
            )

        assign_arr[-1]["duration"] = (
            assign_arr[-1]["end_time"] - assign_arr[-1]["start_time"]
        ).total_seconds()
        if current_assign != []:
            new_assign = {
                "user_id": current_assign[0]["id"],
    ...
    if curr_assign != []:
        ...
        if not user_collection.find_one(filter):
            ...
            user_email = requests.get(
                f"https://code.ethicsinfotech.in/api/v4/users/{user['id']}",
                headers=headers,
            ).json()["email"]
            if not user_email:
    ...
    if prev_assign != []:
        ...
        if not user_collection.find_one(filter):
            ...
            for index in range(len(assign_issue_arr) - 1, -1, -1):
                if (
                    assign_issue_arr[index]["issue_id"] == issue_id
                    and assign_issue_arr[index]["start_time"]
                    and not assign_issue_arr[index]["end_time"]
                ):
                    assign_issue_arr[index]["end_time"] = curr_time
                    # convert to datetime.datetime from string
                    assign_issue_arr[index]["start_time"] = datetime.strptime(
                        assign_issue_arr[index]["start_time"], "%Y-%m-%dT%H:%M:%S.%f"
                    )
                    assign_issue_arr[index]["duration"] = (
                        curr_time - assign_issue_arr[index]["start_time"]
                    ).total_seconds()
...
def end_qa_assign(payload, db):
    ...
    for index in range(len(assign_arr) - 1, -1, -1):
        if assign_arr[index]["end_time"] == None:
            ...
            assign_arr[index]["duration"] = (
                curr_time - assign_arr[index]["start_time"]
            ).total_seconds()
            break
    update = {"$set": {"assign": assign_arr}}
    issue_collection.update_one(filter, update)
    ...
    for index in range(len(assign_issue_arr) - 1, -1, -1):
        if (
            assign_issue_arr[index]["issue_id"] == payload["object_attributes"]["id"]
            and assign_issue_arr[index]["end_time"] is None
        ):
            assign_issue_arr[index]["end_time"] = curr_time
            assign_issue_arr[index]["duration"] = (
                curr_time - assign_issue_arr[index]["start_time"]
