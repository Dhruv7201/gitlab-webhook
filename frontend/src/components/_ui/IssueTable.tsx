...
<div className="relative ">
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
      {issueData && issueData.data.map((issue)=>{
        return (
          <tr className="bg-white dark:bg-gray-800 border-b" key={issue.issue_name}>
            <th scope="row" className="px-6 py-4  whitespace-nowrap dark:text-white cursor-default	">
              {
                issue.issue_name.length > 100?
                <a title={issue.issue_name} className="font-medium text-gray-900 truncate">   
                  {issue.issue_name}
                </a> 
                :
                ...
                <div>
                  created {renderLabel(issue.created_at)} ago by {issue.creator_name}
                </div>
            </th>
